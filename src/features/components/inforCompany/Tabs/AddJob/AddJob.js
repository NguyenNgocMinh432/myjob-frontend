...
export default function AddJob({ id }) {
	...
	const { tagId, price1, price2, nature, request, date, load, typeofworkId, address } = state;
	const onSubmit = (data) => {
		if (
			data.name === "" ||
			price1 === "" ||
				...
				<form onSubmit={handleSubmit(onSubmit)}>
					<div className="d-flex">
						...
						<div className="form-group w-45">
							...
							<Select value={address} onChange={onchangeAddress} className="form-control w-100">
								{getProvinces().map((data, index) => (
									<Option value={FormatProvince(data.name)} key={index}>{data.name}</Option>
								))}
							</Select>
						</div>
					</div>
					...
					<div className="form-group">
						...
						<JoditEditor value={description} tabIndex={1} onChange={(e) => setDescripton(e)} />
					</div>
					<div className="form-group">
						<label htmlFor="">quyền lợi được hưởng</label>
						<JoditEditor value={interest} tabIndex={1} onChange={(e) => setInterest(e)} />
					</div>
					<div className="form-group">
						<label htmlFor="">Yêu cầu công việc</label>
						<JoditEditor value={form} tabIndex={1} onChange={(e) => setForm(e)} />
					</div>
					<div className="form-group">
						<label htmlFor="">Yêu cầu kinh nghiệm</label>
						<JoditEditor value={exprience} tabIndex={1} onChange={(e) => setExprience(e)} />
					</div>
					{load ? (
						<SpinLoad />
					) : (
