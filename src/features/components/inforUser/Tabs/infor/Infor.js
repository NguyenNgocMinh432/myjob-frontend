...
export default function Infor({ id }) {
	...
	const onSubmit = async (data) => {
		console.log(data);
		if (data.phone === "" || tagId.length === 0 || typeofworkId.length === 0 || content === "") {
			...
			if (img !== "" || imgBanner !== "") {
				if (img !== "" && imgBanner === "") {
					...
					const anhBanner = await storage.ref("imagesuser").child(imgBanner.name).getDownloadURL();
					edit({ data, anhBanner });
				} else {
					await storage.ref(`imagesuser/${img.name}`).put(img);
					const anh = await storage.ref("imagesuser").child(img.name).getDownloadURL();
				...
				<form action="" onSubmit={handleSubmit(onSubmit)}>
					...
					<div className="form-group">
						...
						{linkImgBanner ? (
							...
							<img src={anhBanner} className="ml-5" height="150px" width="250px" alt="" />
						) : (
							""
						)}
						<br />
					...
					<div className="d-flex">
						<div className="form-group w-45">
							...
							<Select value={male} onChange={(e) => setMale(e)} className="form-control w-100">
